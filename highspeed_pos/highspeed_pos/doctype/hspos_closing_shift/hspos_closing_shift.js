...
frappe.ui.form.on('HSPOS Closing Shift', {
	onload: function (frm) {
		...
		frm.set_query("user", function (doc) {
			return {
				query: "highspeed_pos.highspeed_pos.doctype.hspos_closing_shift.hspos_closing_shift.get_cashiers",
				filters: { 'parent': doc.pos_profile }
			};
		});

		frm.set_query("hspos_opening_shift", function (doc) {
	...
	hspos_opening_shift (frm) {
		if (frm.doc.hspos_opening_shift && frm.doc.user) {
			reset_values(frm);
			frappe.run_serially([
	...
	set_opening_amounts (frm) {
		frappe.db.get_doc("HSPOS Opening Shift", frm.doc.hspos_opening_shift)
	...
	get_pos_invoices (frm) {
		frappe.call({
			method: 'highspeed_pos.highspeed_pos.doctype.hspos_closing_shift.hspos_closing_shift.get_pos_invoices',
			args: {
				hspos_opening_shift: frm.doc.hspos_opening_shift,
			},
			callback: (r) => {
	...
	get_pos_payments (frm) {
		frappe.call({
			method: 'highspeed_pos.highspeed_pos.doctype.hspos_closing_shift.hspos_closing_shift.get_payments_entries',
			args: {
				hspos_opening_shift: frm.doc.hspos_opening_shift,
			},
			callback: (r) => {
...
frappe.ui.form.on('HSPOS Closing Shift Detail', {
...
function add_to_payments (d, frm) {
	d.payments.forEach(p => {
		...
		if (payment) {
			let amount = p.amount;
			let cash_mode_of_payment = get_value("POS Profile", frm.doc.pos_profile, 'hspos_cash_mode_of_payment');
