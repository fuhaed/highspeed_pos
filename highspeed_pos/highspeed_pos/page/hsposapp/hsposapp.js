// Include onscan.js
frappe.pages['hsposapp'].on_page_load = function (wrapper) {
	...
	this.page.$PosApp = new frappe.PosApp.hsposapp(this.page);
		...
		const update_totals_based_on_tax_inclusive = () => {
			...
			frappe.call({
				...
				args: {
					doctype: 'POS Profile',
					name: posProfile,
					fieldname: 'hspos_tax_inclusive'
				},
				callback: function(response) {
					if (response.message !== undefined) {
						const hspos_tax_inclusive = response.message;
...
if (frappe.boot.lang == "pt") {
	$.extend(
		...
		"Create HSPOS Opening Shift": "Criar Turno de Abertura POS",
