...
from highspeed_pos.highspeed_pos.api.m_pesa import submit_mpesa_payment
...
def process_pos_payment(payload):
    ...
    if not data.pos_profile.get("hspos_use_highspeed_pos_payments"):
        frappe.throw(_("HIGHSPEED POS Payments is not enabled for this POS Profile"))
    ...
    if not data.hspos_opening_shift_name:
        frappe.throw(_("HSPOS Opening Shift is required"))

    company = data.company
    currency = data.currency
    customer = data.customer
    hspos_opening_shift_name = data.hspos_opening_shift_name
    allow_make_new_payments = data.pos_profile.get("hspos_allow_make_new_payments")
    allow_reconcile_payments = data.pos_profile.get("hspos_allow_reconcile_payments")
    allow_mpesa_reconcile_payments = data.pos_profile.get(
        "hspos_allow_mpesa_reconcile_payments"
    )
    today = nowdate()
        ...
        for payment_method in data.payment_methods:
            try:
                ...
                new_payment_entry = create_payment_entry(
                    ...
                    posting_date=today,
                    reference_no=hspos_opening_shift_name,
            ...
            if created_payment and new_payment_entry:
                ...
                try:
                    ...
                    if not mode_of_payment:
                        ...
                        default_cash_mop = data.pos_profile.get("hspos_cash_mode_of_payment")
                        if default_cash_mop:
...
def on_payment_entry_cancel(doc, method):
    try:
        ...
        linked_je = doc.get("hspos_linked_je")
        if not linked_je:
            return
...
def setup_payment_entry_cancel_hook():
    ...
    try:
        ...
        custom_field_exists = frappe.db.exists(
            "Custom Field", 
            {"dt": "Payment Entry", "fieldname": "hspos_linked_je"}
        )
        ...
        if not custom_field_exists:
            try:
                ...
                field = frappe.get_doc({
                    ...
                    "dt": "Payment Entry",
                    "fieldname": "hspos_linked_je",
                    "label": "HIGHSPEED POS Linked JE",
                    "fieldtype": "Link",
                    "options": "Journal Entry",
                    "read_only": 1,
                    "hidden": 0,
                    "description": "Linked Journal Entry created by HIGHSPEED POS",
            ...
            frappe.log_error(f"Custom field 'hspos_linked_je' already exists for Payment Entry", "POS Setup")
...
def fix_payment_entry_links():
    ...
    try:
        ...
        for pe in payment_entries:
            ...
            if frappe.db.get_value("Payment Entry", pe.name, "hspos_linked_je"):
                continue
            ...
            if je_list:
                for je in je_list:
                    ...
                    if comments:
                        ...
                        frappe.db.set_value("Payment Entry", pe.name, "hspos_linked_je", je.name)
